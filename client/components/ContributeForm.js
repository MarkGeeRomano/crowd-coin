        ...
        {hasAddress ?
          <div style={{ maxHeight: this.state.height }} className={styles.container}>
            <form autoComplete='off' onSubmit={this.onSubmit.bind(this)}>
              <label htmlFor='donate'>{web3.utils.fromWei(minContribution, 'ether')}<div className='ether-denom'></div> to gain voting rights</label>
              <input id='donate' value={contribution} onChange={this.onChange.bind(this)} />
              <br />
              <button >{loading ? <div className={styles.loader}></div> : 'Contribute'}</button>
              {msg &&
